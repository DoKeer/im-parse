...
class CustomTableLayout(
    ...
    private fun createCellView(cell: TableCellNode): TextView {
        ...
        for (child in cell.children) {
            InlineNodeRenderer.appendInlineNode(spannable, child, renderContext, mathNodes, displayMetrics, textView)
        }
        ...
        if (mathNodes.isNotEmpty()) {
            InlineNodeRenderer.renderInlineMathNodes(textView, spannable, mathNodes, renderContext)
        }
    ...
    private fun computeColumnPrefWidths() {
        ...
        for (row in tableNode.rows) {
            for ((cellIndex, cell) in row.cells.withIndex()) {
                ...
                for (child in cell.children) {
                    InlineNodeRenderer.appendInlineNode(spannable, child, renderContext, mathNodes)
                }
    ...
    // 注意：appendInlineNode、applyTextStylesToSpan、renderInlineMathNodes、applyMathRenderResults
    // 等方法已移至 InlineNodeRenderer 工具类，直接调用工具类方法即可
}
