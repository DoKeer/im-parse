...
impl ASTBuilder {
    ...
    fn flush_text_buffer(&mut self) {
        ...
        // 检查是否包含特殊语法
        let has_special_syntax = content.contains('$') 
            || content.contains("==") 
            || content.contains('^') 
            || content.contains('~');
        
        if has_special_syntax {
            // 使用统一的 InlineSyntaxParser 解析所有行内语法
            use crate::text_span::{InlineSyntaxParser, InlineSyntaxSpan};
            
            let syntax_spans = InlineSyntaxParser::parse(&content);
            
            // 将解析结果转换为 AST 节点
            for span in syntax_spans {
                match span {
                    InlineSyntaxSpan::BlockMath { range } => {
                        self.current_paragraph_children.push(ASTNode::MathBlock(MathNode {
                            content: content[range].to_string(),
                        }));
                    }
                    
                    InlineSyntaxSpan::InlineMath { range } => {
                        self.current_paragraph_children.push(ASTNode::InlineMath(MathNode {
                            content: content[range].to_string(),
                        }));
                    }
                    
                    InlineSyntaxSpan::Superscript { range } => {
                        let mut combined_styles = styles.clone();
                        combined_styles.push(TextStyle::Superscript);
                    ...
                    InlineSyntaxSpan::Subscript { range } => {
                        let mut combined_styles = styles.clone();
                        combined_styles.push(TextStyle::Subscript);
                        self.current_paragraph_children.push(ASTNode::Text(TextRun::with_styles(
                            content[range].to_string(),
                            combined_styles,
                        )));
                    ...
                    InlineSyntaxSpan::Highlight { range } => {
                        let mut combined_styles = styles.clone();
                        combined_styles.push(TextStyle::BackgroundColor { color: "#FFFF00".to_string() });
                        self.current_paragraph_children.push(ASTNode::Text(TextRun::with_styles(
                            content[range].to_string(),
                            combined_styles,
                        )));
                    ...
                    InlineSyntaxSpan::Text { range } => {
                        let text_content = &content[range];
                        if !text_content.is_empty() {
                            let text_run = if styles.is_empty() {
                                TextRun::new(text_content.to_string())
                            } else {
                                TextRun::with_styles(text_content.to_string(), styles.clone())
                            };
